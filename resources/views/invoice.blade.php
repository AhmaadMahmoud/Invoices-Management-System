                        ...
                        <div class="row mg-t-20">
                            ...
                            <div class="col-md">
                                ...
                                <p class="invoice-info-row"><span>رقم الفاتورة</span>
                                    <span>{{ $printInvoice->invoice_number }}</span></p>
                                <p class="invoice-info-row"><span>تاريخ الاصدار</span>
                                    <span>{{ $printInvoice->invoice_Date }}</span></p>
                                <p class="invoice-info-row"><span>تاريخ الاستحقاق</span>
                                    <span>{{ $printInvoice->due_date }}</span></p>
                                <p class="invoice-info-row"><span>القسم</span>
                                    <span>{{ $printInvoice->section->section_name }}</span></p>
                            </div>
                        </div>
                        <div class="table-responsive mg-t-40">
                            <table class="table table-invoice border text-md-nowrap mb-0">
                                ...
                                <tbody>
                                    <tr>
                                        <td>1</td>
                                        <td class="tx-12">{{ $printInvoice->product }}</td>
                                        <td class="tx-center">{{ number_format($printInvoice->amount_collection, 2) }}</td>
                                        <td class="tx-right">{{ number_format($printInvoice->amount_Commission, 2) }}</td>
                                        @php
                                        $total = $printInvoice->amount_collection + $printInvoice->amount_Commission ;
                                        @endphp
                                        <td class="tx-right">
                                    ...
                                    <tr>
                                        <td class="tx-right">نسبة الضريبة ({{ $printInvoice->rate_VAT }})</td>
                                        <td class="tx-right" colspan="2">287.50</td>
                                    </tr>
                                    <tr>
                                        <td class="tx-right">قيمة الخصم</td>
                                        <td class="tx-right" colspan="2"> {{ number_format($printInvoice->discount, 2) }}</td>

                                    </tr>
                                    <tr>
                                        <td class="tx-right tx-uppercase tx-bold tx-inverse">الاجمالي شامل الضريبة</td>
                                        <td class="tx-right" colspan="2">
                                            <h4 class="tx-primary tx-bold">{{ number_format($printInvoice->total, 2) }}</h4>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
